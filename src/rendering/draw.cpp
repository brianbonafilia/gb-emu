...
bool IsInWindow(const PpuState &state) {
  if (!state.registers.window_enable || !state.registers.wy_eq
      || !state.registers.wx_eq) {
    return false;
  }
...
void DrawDebugScreen(const PpuState &state) {
  ...
  for (int row = 0; row < 64; ++row) {
    for (int col = 0; col < 32; ++col) {
      ...
      DrawDebugTile(state, vram_location, col * 8, row * 8, pixels, 0);
    }
  }
...
void DrawDot(const PpuState &state) {
  ...
  // in order for window to turn on in a frame at one point WX must be
  // equal to X position
  if (x_pos == state.registers.WX) {
    state.registers.wx_eq = true;
  }
